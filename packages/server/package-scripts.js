...
function eslint(path, shouldFix = false) {
	let fix = "";
	if (shouldFix) fix = "--fix";
	return `eslint ${path} ${fix}`;
}

function prettier(path, shouldFix = false) {
	let fix = "--check";
	if (shouldFix) fix = "--write";
	return `prettier --ignorePath .gitignore ${path} ${fix}`;
}
...
module.exports = {
	scripts: {
		...
		lint: {
			...
			hiddenFromHelp: true,
			fix: eslint(`./src/**/*.ts`, true)
		},
		/**
		 * Runs Prettier over your project
		 */
		prettier: {
			script: prettier(`./src/**/*.ts`),
			fix: prettier(`./src/**/*.ts`, true)
		},
		/**
		...
		db: {
			...
			seed: {
				script: series(
					...
					"nps config",
					runFast(
						"../../node_modules/typeorm-seeding/dist/cli.js seed"
					)
				),
			...
			drop: {
				script: runFast(
					"../../node_modules/typeorm/cli.js schema:drop"
				),
